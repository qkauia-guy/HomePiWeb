...
5. 填入基本表單
6. 註冊成功
7. 轉跳<u>預設</u>進入建立群組頁面
8. 發送註冊成功通知(個人)

###### ▶ 新設備開通-群組共享(SuperAdmin)：
...
###### ▶ 後台群組管理-建立(SuperAdmin)：

0. 後台管理頁面
1. 進入群組管理(<u>預設</u>群組列表)
2. 點選建立群組
3. 輸入群組表單
4. 建立群組成功
5. 轉跳<u>預設</u>群組列表
...
3. SuperAdmin 在後台群主管理/成員管理/選擇該用戶重設密碼
4. 輸入預設密碼登入
5. 轉跳修改密碼頁面
6. 修改密碼
7. 發送修改密碼成功通知(個人)
...
以下是根據你原本的 22 頁流程，整併後最終建議的 13 個實體頁面，方便實作與展示。
| 編號 | 頁面名稱 | 說明 |
|------|---------------------------------|------|
| 1 | 登入頁面 | 含「忘記密碼」按鈕 |
